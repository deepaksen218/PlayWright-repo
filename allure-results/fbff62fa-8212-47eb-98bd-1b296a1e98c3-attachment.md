...
- Name: API.spec.js >> GET request to fetch Users
- Location: tests\API.spec.js:3:6
...
```ts
  ...
  3  | test.only('GET request to fetch Users', async({request})=>{
  4  |     
  5  |     const response= await request.get('https://jsonplaceholder.typicode.com/users/11');
> 6  |     expect(response.ok()).toBeTruthy();
  ...
  22 |     // for validation of response body
  23 | 
